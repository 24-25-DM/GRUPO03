...
class DatabaseController {
  ...
  final result = await _connection.query(
    'SELECT isActive FROM vehicles WHERE plate = @plate',
  ...
  bool isActive = result.first[0] as bool;

  if (!isActive) {
    throw Exception("Imposible de editar: el vehículo está inactivo.");
  }

  
  final updateResult = await _connection.query(
    'UPDATE vehicles SET plate = @newPlate, brand = @brand, manufactureDate = @manufactureDate, color = @color, cost = @cost, isActive = @isActive, imageUrl = @imageUrl WHERE plate = @plate',
    substitutionValues: {
  ...
  return updateResult.affectedRowCount;
}
